...
```
model: claude-sonnet-5 | effort: low | permission: bypassPermissions | cwd: /home/adrian/projects/ai-inventor/aii_data/users/uitest-20260731/runs/run_0WmBa7GFLIzI/3_invention_loop/iter_1/gen_plan/gen_plan_evaluation_1
```
...
approach: >-
  ...
  run paired significance tests (e.g. McNemar or bootstrap CI) between checksum vs free-form and checksum vs placebo on the
  checksum-detectable subset; separately report performance on checksum-invisible problems to show the method's ceiling; audit
  a sample of checksum-critique traces to measure how often the model miscomputes its own digit-root checksum (false alarms/misses
  caused by checksum-computation errors rather than the invariant itself failing).
depends_on: []
</artifact_direction>
